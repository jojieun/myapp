...
{
    // 리뷰어입장
    public function index(Advertiser $advertiser, Reviewer $reviewer)
    {
        $messages = Message::where(function($query) use ($advertiser, $reviewer) {
        ...
        Message::where(function($query) use ($advertiser, $reviewer) {
            $query->where('advertiser_id', $advertiser->id);
            $query->where('reviewer_id', $reviewer->id);
    ...
    //광고주입장
    public function index2(Advertiser $advertiser, Reviewer $reviewer)
    {
        $messages = Message::where(function($query) use ($advertiser, $reviewer) {
            $query->where('advertiser_id', $advertiser->id);
            $query->where('reviewer_id', $reviewer->id);
