...
<table>
    ...
    @forelse ($banners as $banner)
        <tr>
            <form method="post" action="{{route('admin.main_banner_modi',$banner->id)}}" enctype="multipart/form-data">
                {!! csrf_field() !!}
        ...
        @endforelse
<tr>
    <form method="post" action="{{route('admin.main_banner_add')}}" enctype="multipart/form-data">
        {!! csrf_field() !!}
                <td>+</td>
...
(*** 배너 이미지 크기 : 1920 * 530 px)
<script>
    //삭제 클릭시
    $('.del').on('click', function(e){
       e.preventDefault();
        location.href = "main_banner_del/"+$(this).val();
    });
    //    이미지업로드바로보기
