...
<form action="{{ route('reviewers.social_store') }}" method="post" class="form__auth">
{!! csrf_field() !!}
...
<!--             //popup : 필수약관 동의 필요-->
		...
		<div class="content-in-sub member" id="content_2">
			...
			<div class="join-agreement">
				<h3>필수정보입력</h3>
                <input type="hidden" name="email" placeholder="이메일" value="{{$social_email}}">
                <input type="hidden" name="name" placeholder="이름" value="{{$social_name}}">
